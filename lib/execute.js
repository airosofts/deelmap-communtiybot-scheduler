...
import {
  ...
  classifyLotForContent, recentBotTitles, angleSeed,
} from './bot-content.js'
...
export async function executeSchedule({ schedule, settings }) {
  ...
  // Variety injection — feed Groq the bot's recent titles + a random framing
  // angle so back-to-back runs stop producing the same headline.
  let varietyContext = ''
  if (wantsTitle) {
    const recent = await recentBotTitles({
      botProfileId: bot.profile_id,
      lotId: scenario.target_lot_id || null,
      days: 7, limit: 12,
    })
    const angle = angleSeed()
    varietyContext = `
VARIETY RULES — read carefully:
• Frame this post around: ${angle}
• Your title MUST be distinctly different from any of your recent titles.
• Do NOT reuse the same opening phrase, same city, or same deal type as last time unless the brief specifically requires it.
${recent.length ? `
RECENT TITLES YOU (or peer bots) HAVE ALREADY USED — DO NOT repeat, paraphrase, or copy the structure of any of these:
${recent.map(t => `  • ${t}`).join('\n')}` : ''}
`
  }

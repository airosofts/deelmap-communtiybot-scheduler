...
export async function executeSchedule({ schedule, settings }) {
  ...
  // NOTE: jitter is NOT a blocking sleep — it's baked into next_run_at by
  // bumpScheduleCounters so the tick loop never stalls and other schedules
  // keep firing. The run happens immediately when due.
  ...
  if (scenario.action_type === 'comment') {
    ...
    if (!commentTarget) {
      ...
      await advanceNextRunOnly(schedule)
...
${formatInstr}`,
  ...
  try {
    ...
    // Post / reply_chain
    if (!scenario.target_lot_id) throw new Error('No target_lot_id.')
...
// A real run happened — advance next_run_at AND count it toward the daily cap.
...
// A skip happened (no target etc.) — push next_run_at forward but DON'T count
// it as a run, so the daily cap isn't wasted and "Last run" stays accurate.
async function advanceNextRunOnly(schedule) {

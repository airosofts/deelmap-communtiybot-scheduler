...
// Recent titles by this bot in this lot — fed to Groq with "do not repeat" so
// the AI stops converging on the same headline. Falls back to titles by any
// bot in the lot if this bot has no history yet.
export async function recentBotTitles({ botProfileId, lotId = null, days = 7, limit = 12 }) {
  const since = new Date(Date.now() - days * 86_400_000).toISOString()
  let q = supabase
    .from('community_posts').select('title')
    .eq('author_id', botProfileId).gte('created_at', since)
    .order('created_at', { ascending: false }).limit(limit)
  if (lotId) q = q.eq('lot_id', lotId)
  const { data: own } = await q
  if (own?.length) return own.map(p => p.title).filter(Boolean)

  if (lotId) {
    const { data: peers } = await supabase
      .from('community_posts').select('title')
      .eq('lot_id', lotId).gte('created_at', since)
      .order('created_at', { ascending: false }).limit(limit)
    return (peers || []).map(p => p.title).filter(Boolean)
  }
  return []
}

const ANGLES = [
  'a specific city or metro you have NOT mentioned in your recent titles',
  'the financing/loan angle rather than the deal itself',
  'a mistake, close-call, or lesson learned',
  'the exit strategy or refinance outcome',
  'an unexpected expense or surprise during the deal',
  'comparing two different markets, lenders, or approaches',
  'a contractor, vendor, or property-manager experience',
  'the math behind the deal with specific numbers',
  'a sharp question to the room rather than a brag',
  'a contrarian take or pushback on common advice',
  'a tax, entity, or structure angle',
  'a regional market shift or trend you are watching',
  'a tenant, lease, or operations story',
  'the timing — why now vs. last year vs. next year',
  'the source of the deal (broker, off-market, MLS, network)',
]
export function angleSeed() {
